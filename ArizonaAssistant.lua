...
function sampev.onSendSpawn()
	if auth_status == false then
		sampAddChatMessage('[Èíôîðìàöèÿ] {7B68EE}Äëÿ àâòîðèçàöèè èñïîëüçóéòå {FF0000}/auth', 0xDAA520)
	end
end
...
function main()
  ...
  while true do
  	wait(10)
  	if auth_status then
    	if isKeyJustPressed(VK_F2) then -- Ãëàâíîå ìåíþ
        imgui.Process = true
...
function cmd_auth()
	if auth_status then
		sampAddChatMessage('[Èíôîðìàöèÿ] {7B68EE}Âû óæå àâòîðèçîâàíû!', 0xDAA520)
	else
    print('Starting load...')
    load_startup_images()
    sampAddChatMessage('[Èíôîðìàöèÿ] {7B68EE}Ñêðèïò óñïåøíî çàãðóæåí!', 0xDAA520)
		auth_status = true
	end
end
...
function imgui.OnDrawFrame()
  if starup_window.v then
    ...
    imgui.SetNextWindowSize(imgui.ImVec2(333, 333), imgui.Cond.FirstUseEver) -- ìåíÿåì ðàçìåð =============
    imgui.SetNextWindowPos(imgui.ImVec2(333, 333))
    imgui.Begin(u8'Ãëàâíîå ìåíþ', use_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoCollapse)
